...
/**
 * The room: a floor, a ring of wall panels, two interior stubs, a handful of pushable crates, and
 * three **interactable** pedestals the crosshair ray can find.
 *
 ...
 * each panel is its own collider.
 *
...
/** Half the room, in metres: the floor spans `[-12, 12]` on both axes. */
...
/** Where the three interactable pedestals stand, in metres. Hand-placed, not scattered. */
const PEDESTALS: readonly (readonly [number, number])[] = Object.freeze([
  [-4, -6],
  [4, -6],
  [0, 6],
]);

/** The edge of a pedestal, in metres. */
const PEDESTAL_SIZE = 0.9;
...
/**
 * Adds one wall panel: a mesh instance and a static collider.
 *
 ...
 * @returns The crates and the interactable pedestals.
 */
export function buildLevel(app: App, materials: LevelMaterials): Level {
  ...
  for (let index = 0; index < perSide; index += 1) {
    ...
    addPanel(app, alongX, materials.wall, `Wall N${String(index)}`, offset, -ARENA_HALF);
    addPanel(app, alongX, materials.wall, `Wall S${String(index)}`, offset, ARENA_HALF);
    addPanel(app, alongZ, materials.wall, `Wall W${String(index)}`, -ARENA_HALF, offset);
    addPanel(app, alongZ, materials.wall, `Wall E${String(index)}`, ARENA_HALF, offset);
  }
  ...
  // The pedestals. Each carries its **own** clone of the crate material, because
  // `setBaseColor` writes the Lite material every renderer sharing the asset draws with: without
  // the clone, lighting one pedestal would light all three.
  const interactableLayer = app.world.layers.requireIndex("Interactable");
  const pedestalMesh = MeshAsset.box(app, { size: PEDESTAL_SIZE });
  const pedestals: Interactable[] = [];
  for (let index = 0; index < PEDESTALS.length; index += 1) {
    const spot = PEDESTALS[index];
    if (spot === undefined) {
      continue;
    }
    const material = materials.crate.value.clone(app);
    const pedestal = app.world.createEntity(`Pedestal ${String(index)}`, {
      position: { x: spot[0], y: PEDESTAL_Y, z: spot[1] },
    });
    pedestal.layer = interactableLayer;
    pedestal.addComponent(MeshRenderer, {
      mesh: pedestalMesh.retain(),
      materials: [material.retain()],
      castShadows: true,
      receiveShadows: true,
    });
    pedestal.addComponent(BoxCollider, { size: { x: PEDESTAL_SIZE, y: PEDESTAL_SIZE, z: PEDESTAL_SIZE } });
    // A post under it, so a pedestal reads as furniture rather than as a floating box.
    const post = app.world.createEntity(`Pedestal ${String(index)} Post`, {
      position: { x: spot[0], y: (PEDESTAL_Y - PEDESTAL_SIZE / 2) / 2, z: spot[1] },
    });
    post.layer = levelLayer;
    post.addComponent(MeshRenderer, {
      mesh: MeshAsset.box(app, { width: 0.3, height: PEDESTAL_Y - PEDESTAL_SIZE / 2, depth: 0.3 }),
