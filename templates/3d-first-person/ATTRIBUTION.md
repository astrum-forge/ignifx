...
| File                                 | What it is                                                                                                         |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| `viewmodel.glb`                      | One copy of a generated four-joint "box-man" rig with `idle`/`walk`/`run`/`jump` clips and a `hand` node.          |
...
| `blip.wav`                           | Two decaying sine partials with a click at the front, synthesised sample by sample.                                |
...
uses `node:zlib` and no image or audio library at all. The template carries one copy of the rig because
it needs only one animated character; `@ignifx/3d` binds one `Animator` per **model asset**, so a
second animated character would need a second address.
