 ...
 * the pointer.
 *
 * `ThirdPersonCamera` is built with `lockPointerOnClick: true`, so an unlocked mouse deliberately
 * does **not** orbit the camera: a cursor crossing the canvas on its way to a menu button is not a
...
export class LockHint extends Script implements ScriptCallbacks {
  static typeId = "third-person/LockHint";
