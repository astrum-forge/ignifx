...
/** The brand-mark PNG sizes `website/public/brand/` ships (`05-press-kit.md` §3). */
 ...
 * The ignifx mark.
 *
 * Since the owner supplied a professionally designed icon (`02-design-system.md` §2.1,
 * `08-execution.md` §9) the artwork is **raster**: a 1024 px PNG on a transparent, ink-centred square
 * canvas. It is therefore an `<img>` with a 2× `srcset` rather than an inline path, and it is the one
 * raster image in the site's chrome — exempt from the "every image is a capture" rule and, being above
 * the fold wherever it appears, from `loading="lazy"`.
 *
 ...
 *
 * @param size - The 1× size to request; the 2× source is the next size up.
 * @param className - Extra class.
 * @returns The `<img>` HTML.
 */
export function markImg(size: MarkSize, className = "mark"): string {
  const index = MARK_SIZES.indexOf(size);
  const retina = MARK_SIZES[index + 1] ?? size;
  return h("img", {
    class: className,
    src: `/brand/mark-${String(size)}.png`,
    srcset: `/brand/mark-${String(size)}.png 1x, /brand/mark-${String(retina)}.png 2x`,
    width: size,
    height: size,
    alt: "",
    "aria-hidden": "true",
