 ...
 * contents, links rewritten by the caller's resolver, scroll containers around tables, and a copy
  ...
  marked.use({
    renderer: {
      code({ text, lang }: Tokens.Code): string {
        return highlighter.render(text, lang ?? "", { colour: !compact });
      },
      ...
      html({ text }: Tokens.HTML | Tokens.Tag): string {
        ...
        // A generated page opens with a `<!-- Generated by … -->` marker. It is a note to whoever
        // edits the file, not content, so it is dropped rather than escaped into the prose.
        if (trimmed.startsWith("<!--")) {
          return "";
        }
        return ALLOWED_HTML.test(trimmed) ? text : esc(text);
      },
    },
  });
