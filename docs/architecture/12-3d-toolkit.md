...
- Requires `CharacterController`. `Look` action drives yaw on the entity and pitch on a child camera entity (`cameraPivot`), clamped to ±89°; pointer lock requested on a `pointerdown` of the frame (`app.input.pointerLock`), which is the gesture browsers accept as user activation; head bob and FOV kick optional and off by default; crouch via `CharacterController.setHeight(height, preserveFeet)`.

**Correction (2026-09-08, the defects behind "first person is jumpy and fiddly").** The look half of both rigs now lives in one internal module, `src/character/look-input.ts`, and it changed in four ways.

1. **The lock request re-arms.** It used to be one-shot: a latch was set on the first request and never cleared, so once the browser released the lock — Escape, which is also the pause key, or a focus change — no later click could take it again and the player finished the session with an unlocked mouse. The request is now issued on **every** frame that carries a `pointerdown` while `lockPointerOnClick` is on and the lock is not held; the only guard is the request already in flight. A refusal, including the `IGX-0809` a headless app rejects with, is logged and dropped.
2. **Unlocked mouse look is ignored.** With `lockPointerOnClick` on and the lock not held, a `Look` reading whose `InputAction.activeDevice` is `Mouse` or `Pointer` is discarded. A cursor crossing the canvas on its way to a menu button was turning the view, and the turn stopped dead when the cursor left the frame. Gamepad, touch, and virtual sticks are never gated: they have no cursor to lose. `lockPointerOnClick: false` keeps unconditional mouse look, which is what a drag-to-look design wants.
3. **A stick is a rate, a pointer is a displacement.** `sensitivity` is degrees per unit of **pointer** look — with CSS-pixel deltas (`08-input.md` §5) that is degrees per CSS pixel, DPR- and render-scale-independent, and 0.08–0.15 suits most mice. A gamepad or virtual stick reports a deflection, so it is multiplied by the new `stickLookSpeed` field (degrees per second at full deflection, default 180) **and the frame delta**: `yaw += look.x * stickLookSpeed * dt`. Before, a stick turned twice as far at 120 Hz as at 60 Hz and games papered over it with a `scale(18)` processor. The delta is the scaled one `update`/`lateUpdate` receive, so slow motion slows the look with everything else; a paused app stops calling `update` on the rigs altogether unless one declares `updateWhenPaused` (§7 of `01-lifecycle-and-time.md`).
4. **Up is up on every device.** The two device families disagree about which way `y` points — a screen's `movementY` grows downward, a stick's `y` grows upward — so one `invertY` could only ever suit one of them, and the first-person template's mouse shipped inverted as a result. `look-input.ts` negates the pitch axis for `Mouse`, `Pointer`, and `Touch` readings and leaves gamepad and virtual alone, so what it hands a rig always means "`+y` is up"; the rigs apply their own `-look.y` and `invertY` on top, unchanged. Moving the mouse forward and pushing a stick up now both look **up** in first person, which is the FPS convention; in third person both lower the boom and aim the camera up, and `invertY` gives the swing-up-and-over feel to a game that prefers it.
...
**Correction (2026-09-08, orbit).** The rig's rotation is `Ry(yaw) * Rx(pitch)` — yaw about the world's up, then pitch about the camera's own right — built from two quaternions. A single `Quat.fromEulerDegrees(pitch, yaw, 0)` composes in intrinsic XYZ and yaws about the already-pitched axis, which rolled the horizon and moved the camera up and down as it orbited at any non-zero pitch. The starting yaw and pitch are read from the entity's forward vector at `awake`, not from `Transform.eulerAngles`.

**Correction (2026-09-08).** The boom's sweep passes `ignore: target` to `app.physics.shapeCast`: the shoulder pivot sits inside the character's capsule, Lite's sweep cannot be layer-filtered, and without it the capsule was reported at fraction zero for every yaw whose boom crossed it — the camera collapsed into the character's head. `collisionLayers` decides which hit is _attributed_ an entity; any other body in the way still shortens the boom. `ThirdPersonController`'s step probe ignores its own entity for the same reason, without which `stepHeight` never lifted anything.
...
**Correction (2026-09-08).** The rig shares `src/character/look-input.ts` with `FirstPersonController` and gains the same two fields. `lockPointerOnClick` (**default `false`**) gives a third-person game the console-style "click to capture, Escape to release" behaviour the first-person controller has: while it is on, a `pointerdown` asks for the lock whenever it is not held, and mouse or unified-pointer look is ignored until it is granted. It defaults to `false` because a rig that orbits on a held mouse button needs the cursor, and because that is what the existing third-person example does. `stickLookSpeed` (degrees per second at full deflection, default 180) makes gamepad and virtual-stick orbit a rate rather than a per-frame displacement; `sensitivity` still applies to pointer deltas, now in degrees per CSS pixel.

Which way the mouse pitches the rig, so a template can choose `invertY` deliberately: positive `pitch` **raises** the camera and aims it down at the target, and the look axis is normalised per device (§1.2, point 4), so moving the mouse forward and pushing a stick up both **lower** the boom and aim the camera up, while pulling back raises it and looks down over the target's shoulder. `invertY` flips every device together; a rig that should swing up and over on a forward push sets it.
