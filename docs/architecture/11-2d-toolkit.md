...
- **A trigger is never an obstacle to `CharacterController2D`.** Rapier's kinematic character controller counts sensors as walls unless `computeColliderMovement` is given `QueryFilterFlags.EXCLUDE_SENSORS`, so the adapter always passes it (measured 2026-09-08, ADR-0006 Validation). A character therefore walks through a trigger volume at full speed, `onTriggerEnter`/`onTriggerExit` fire on both entities, and a sensor never appears in `CharacterController2D.onCollided`.
- **The display pose is written at the top of `Update`, not in `PreRender`** (2026-09-08). The interpolation system registers at `Systems(Update, order −900)` in both physics extensions, so `Camera2DFollow` and every hand-written follow camera in `update`/`lateUpdate` read the same pose the renderer draws; see `01-lifecycle-and-time.md` §3 and `09-physics.md` §1.
