...
Each phase lists **Spikes** (retire risk first), **Deliverables**, **Exit criteria** (all must be true and verified in CI, except criteria explicitly marked *manual*, which are recorded in a tracking issue), and **Docs**.
...
- Lint/format: `oxlint.json`, `eslint.config.ts` (gap rules + placeholders for the custom `ignifx/*` rules, implemented in Phase 1), `.oxfmtrc`.
