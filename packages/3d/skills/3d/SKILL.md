...
| `ThirdPersonCamera`     | Orbit rig with damping and wall collision.                       | `target`, `distance`, `minPitch`, `maxPitch`, `sensitivity`, `stickLookSpeed`, `lockPointerOnClick`, `damping`, `shoulderOffset`, `collisionEnabled`, `collisionRadius`, `collisionLayers`; methods `snap()` (after a teleport), read `yaw`, `currentDistance` |
