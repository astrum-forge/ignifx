...
/** The value surface re-exported from `@ignifx/particles`. */
const expectedParticlesExports = [
  "bakeCurve",
  "bakeGradient",
...
/** The value surface re-exported from `@ignifx/particles-2d`. */
const expectedParticles2DExports = [
...
/** The value surface re-exported from `@ignifx/terrain`. */
const expectedTerrainExports = [
  "buildChunkGeometry",
  "chunkBounds",
  "chunkGridSide",
  "chunkIndexCount",
...
describe("ignifx barrel", () => {
  it("imports without executing anything and re-exports the whole @ignifx/core value surface", () => {
    ...
    expect(new Set(Object.keys(barrel))).toEqual(
      new Set([
        ...
        ...expectedParticlesExports,
        ...expectedParticles2DExports,
        ...expectedTerrainExports,
  ...
  it("re-exports the @ignifx/particles value surface by name", () => {
    const keys = new Set(Object.keys(barrel));
    for (const name of expectedParticlesExports) {
      expect(keys.has(name)).toBe(true);
    }
  });

  it("re-exports the @ignifx/particles-2d value surface by name", () => {
    const keys = new Set(Object.keys(barrel));
    for (const name of expectedParticles2DExports) {
      expect(keys.has(name)).toBe(true);
    }
  });

  it("re-exports the @ignifx/terrain value surface by name", () => {
    const keys = new Set(Object.keys(barrel));
    for (const name of expectedTerrainExports) {
      expect(keys.has(name)).toBe(true);
    }
  });
